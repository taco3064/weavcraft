...
function getDataStructure(
  ...
  const { baseChildPath, isStoreWidget, mappableProps, mappingPaths } =
    getCoreProps(widget);
  ...
  children.forEach(({ nodePath, ...childNode }) => {
    const isSubCase = isStoreWidget && nodePath.startsWith(`${baseChildPath}.`);
  ...
  if (!_isEmpty(subDataStructure)) {
    mainDataStructure[Symbol(`${subFieldPath}(${baseChildPath})`)] = [
      subDataStructure,
    ];
  }
...
function getParentStoreNode(
  ...
  const { baseChildPath, isStoreWidget } = getCoreProps(node.widget);
  ...
  if (isStoreWidget) {
    ...
    return {
      node,
      subFieldName: lastPath.startsWith(`${baseChildPath}.`)
        ? `${subFieldPath}(${baseChildPath})`
        : undefined,
    };
  }
